...
# ============ 5. NO FEATURE SCALING (REMOVED) ============
# Random Forest doesn't require scaling - using raw features directly
print("\nUsing raw features (no scaling needed for Random Forest)")
...
axes[1].set_title('Random Forest Permutation Importance (n_repeats=30)')
...
plt.tight_layout()
plt.savefig('random_forest_residuals.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('random_forest_residuals_vs_predicted.png', dpi=300)
plt.show()

# Plot 4: Residuals vs Time (Test Set) - NEW for climate data
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('random_forest_residuals_vs_time.png', dpi=300)
plt.show()
...
# Get predictions for this district (using raw features - no scaling)
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('random_forest_timeseries.png', dpi=300)
plt.show()

# Plot 6: Time Series - Test Period Only (New - demonstrates forecasting ability)
plt.figure(figsize=(14, 6))
...
if len(test_district_data) > 0:
    ...
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('random_forest_timeseries_test_period.png', dpi=300)
    plt.show()
...
try:
    ...
    plt.tight_layout()
    plt.savefig('random_forest_shap_summary.png', dpi=300)
    plt.show()
    
    print("SHAP summary plot saved to 'random_forest_shap_summary.png'")
...
# Save feature importance (renamed from "coefficients")
...
# Save model parameters (not coefficients since RF has no coefficients)
model_params = pd.DataFrame({
    'feature': feature_columns,
    'feature_importance': model.feature_importances_
})
model_params.to_csv('random_forest_model_parameters.csv', index=False)
print("Model parameters saved to 'random_forest_model_parameters.csv'")
...
best_params_df.to_csv('random_forest_best_hyperparameters.csv', index=False)
print("Best hyperparameters saved to 'random_forest_best_hyperparameters.csv'")
...
print("="*50)
print(f"Model Type: Random Forest Regressor (Hyperparameter Tuned)")
...
# ============ 14. LEAKAGE CHECK (Optional Warning) ============
print("\n" + "="*50)
